// 此文件是定义为Count组件服务的reducer(reducer中最好不要写业务逻辑)
...
function countReducer(preState = initState, action) {
  /*
      若初次调用
          (1).preState是undefied，action是内置的一个动作，行如，{type: "@@redux/INITf.4.s.p.r.8"}
          (2).给preState赋初始值，返回preState作为初始状态
      若非初调用。preState是之前的状态，action是一个动作，行如，{type:???,data:???}
